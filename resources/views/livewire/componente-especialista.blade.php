<div class="p-4 grid grid-cols-1 md:grid-cols-12 gap-2">
    ...
    <div class="col-span-12">
        ...
        <h1 class="Text-xl font-bold pt-4 pl-2">Asignaciones</h1>
            ...
            <div class="overflow-x-auto pt-4">
                <table class="table w-full text-gray-400 border-separate space-y-6 text-sm">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-blue-200 text-black">
                            <td class="p-3">{{ $asignacion->propuesta->planificacion->portafolio->nombre }}</td>
                            <td class="p-3">{{ $asignacion->propuesta->planificacion->nombre }}</td>
                            <td class="p-3 ">{{ $asignacion->propuesta->sector->nombre }}</td>
                            <td class="p-3">
                                <a class="flex cursor-pointer text-gray-500 hover:text-gray-100"
                                    wire:click="descargarArchivo({{ $asignacion->propuesta->id }})">
                                    <x-feathericon-hard-drive />
                                    &nbsp;
                                    {{ $asignacion->propuesta->documento->nombre }}
                                </a>
                            </td>
                            <td class="p-3 ">
                                @if ($asignacion->propuesta->prioridad == 1)
                                <div
                                    class="flex justify-center bg-red-500 text-gray-100 items-center p-1 rounded-lg text-base font-bold">
                                    <x-feathericon-alert-triangle /> &nbsp; Alta
                                </div>
                                @else
                                @if ($asignacion->propuesta->prioridad == 2)
                                <div
                                    class="flex justify-center bg-yellow-500 text-gray-100 items-center p-1 rounded-lg text-base font-bold">
                                    <x-feathericon-alert-triangle /> &nbsp; Media
                                </div>
                            ...
                            <td class="flex p-3 items-center">
                                <a wire:click='modalEliminar({{ $asignacion->id }})' class="cursor-pointer">
